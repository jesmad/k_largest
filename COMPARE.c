...
/*  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  *  */
	...
	if (arg1==0) {
		//We're creating the private array
		if (arg2<10 || arg2>COMPARELIMIT) {
			//error message if n is out of range (i.e. n < 10 or n > 1000
			printf("******* ERROR: COMPARE(0,size) -- 'size' out of range ********** size=%d\n",arg2);
			return(-1);
		}
		//number is our private array
		for (i=1; i<=arg2; i++) number[i] = i; // This simply initializes the array with temporary values
		for (i=arg2; i>=1; i--){			  // This puts random values from index number[n] to number[1]
			j = i*dshrandom(0) + 1;
			if (j!=i) {
				t = number[i];
				number[i] = number[j];
		...
		//*-------------------------*
		//Just printing out the values of the private array for practice
		//
			
		printf("The actual unseen elements that COMPARE has access to\n");
		int x;
